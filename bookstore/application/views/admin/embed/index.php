...
<div class='box'>
	<div class='box-header with-border' style='padding: 5px 10px;'>
		...
		<button id='delete' data-id='' class="btn btn-default btn-sm pull-right" onclick='deleteID(this,"embed/delete")'>Xóa Yêu cầu</button>
	</div>
	<div class='box-body'>
		<table class="table table-striped table-bordered datatables table-embed">
		    <thead>
		        <tr>
		            <th style='width: 25px;'><input type="checkbox" class='uniform ckball'></th>
		            <th style='min-width: 130px;'>Thẻ nhúng</th>
		            <th style='min-width: 130px'>Code</th>            
		            <th style='width: 60px;'>Ưu tiên</th>
		            <th style='width: 60px;'>T.Thái</th>
		    ...
		    <?php foreach($_data as $key =>  $d): ?>
		    	<tr>
		            ...
		            <td><a href='<?php echo BASE_URL_ADMIN."embed/add/".$d['id'] ?>'><?php echo $d['title']?></a></td>			            
		            <td><pre><code><?php 
		            $code = str_replace("<", "&lt;", $d['content']);
		            ...
		            <td class='text-center'>
		            	<i class="fa fa-trash-o delete-record" onclick='deleteOne(this,<?php echo $d['id']?>,"embed/delete")'></i>
		            </td>

...
<div class='box'>
	<div class='box-header with-border' style='padding: 5px 10px;'>
		...
		<button type="button" class="btn btn-default btn-sm" onclick="window.location='<?php echo BASE_URL_ADMIN."posts/category_index"?>'">
			<i class="fa fa-reply" aria-hidden="true"></i> <b>Quay lại</b>
		</button>

		<?php if(@$_detail): ?>
		<button type="button" class="btn btn-success btn-sm" onclick="window.location='<?php echo BASE_URL_ADMIN."posts/category_add/" ?>'">
		<i class="fa fa-plus-circle" aria-hidden="true" style=''></i> <b>Tạo mới</b>
		</button>
		<?php endif; ?>
	</div>
	<div class='box-body'>
		<!-- <form class="form-horizontal" action='<?php echo BASE_URL_ADMIN."posts/category_add/".@$_detail['id'] ?>' method='POST' enctype="multipart/form-data">		 -->
		<form class="form-horizontal" action='javascript:void(0)' method='POST' enctype="multipart/form-data" onsubmit="updateAction(this,'menu','<?php echo BASE_URL_ADMIN."posts/category_add/".@$_detail['id'] ?>');return false;">	
